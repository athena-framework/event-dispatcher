...
abstract class Athena::EventDispatcher::EventDispatcherInterface
  ...
  abstract def add_listener(event : Event.class, handler : Proc(Event, EventDispatcherInterface, Nil), priority : Int32) : Nil
  ...
  abstract def get_listeners(event : Event.class | Nil) : Array(Proc(Event, EventDispatcherInterface, Nil))

  # Returns the *listener* priority for the provided *event*.
  abstract def get_listener_priority(event : Event.class, listener : Listener.class) : Int32
